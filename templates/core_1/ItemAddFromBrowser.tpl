...
<div class="gbBlock">
  ...
  <div id="fileDiv_{$iteration}" {if $iteration > $VISIBLE_BOX_COUNT} style="display:none"{/if}>
    <div class="form-group">
      <label class="control-label col-xs-2" for="fileinput_{$iteration}"> {g->text text="File"} </label>
      <input type="file" size="60" id="fileinput_{$iteration}" name="{g->formVar var="form[$iteration]"}"/>
    </div>
    <div class="form-group">
      <label class="control-label col-xs-2" for="captioninput_{$iteration}"> {g->text text="Caption"} </label>
    <textarea rows="4" cols="60" id="captioninput_{$iteration}" name="{g->formVar var="form[caption][$iteration]"}"></textarea>
    </div>
  </div>
  {/section}
