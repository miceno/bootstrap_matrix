    ...
    {else}
        ...
        {if $mapv3.mode eq "Pick" or ($mapv3.mode eq "Normal" and (!isset($mapv3.noiteminalbum) or !$mapv3.noiteminalbum) and (!isset($mapv3.nogpscoords) or !$mapv3.nogpscoords) and (!isset($mapv3.noitemperms) or !$mapv3.noitemperms) and isset($mapv3.googleMapKey) and $mapv3.googleMapKey neq '')} {* number 1 *}
            {if $mapv3.mode eq "Pick" or $mapv3.useMarkerSet <> "none"} {* number 2 *}
                ...
                {/if}
                {if $mapv3.mode eq "Normal" and (!isset($mapv3.LegendPos) or (isset($mapv3.LegendPos) and $mapv3.LegendPos eq 'top')) and $mapv3.fullScreen neq 3}
                ...
                {if $mapv3.mode eq "Normal" and $mapv3.fullScreen neq 3}
                    ...
                    <div class="map-filter">
                        {if isset($mapv3.ShowFilters) and $mapv3.ShowFilters eq "right" and !$mapv3.fullScreen}
                            {g->block type="mapv3.mapFilter"}
                        {/if}
                        {if $mapv3.mode eq "Normal" and isset($mapv3.LegendPos) and $mapv3.LegendPos eq 'right'}
                            {g->block type="mapv3.Legend"}
                        {/if}
                    </div>
                ...
                {if $mapv3.mode eq "Normal" and $mapv3.fullScreen neq 3}
                    ...
                    {if isset($mapv3.ShowFilters) and $mapv3.ShowFilters eq "bottom" and !$mapv3.fullScreen}
                        {g->block type="mapv3.mapFilter"}
                    {/if}
                    {if $mapv3.mode eq "Normal" and (isset($mapv3.LegendPos) and ($mapv3.LegendPos eq 'bottom'))}
                ...
                {if $mapv3.mode eq "Pick"} {* number 3 *}
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-3">
                        ...
                        <form action="{g->url}" method="post" id="itemEdit" enctype="application/x-www-form-urlencoded">
                            ...
                            <input type="hidden" id="coord" name="{g->formVar var="form[coord]"}"
                                   value="{if $mapv3.centerLongLat neq 'none'}{$mapv3.centerLongLat}{else}-12,20{/if}"/>
                            <input type="hidden" id="zoom" name="{g->formVar var="form[zoom]"}"
                                   value="{if $mapv3.zoomLevel neq 'none'}{$mapv3.zoomLevel}{else}16{/if}"/>
